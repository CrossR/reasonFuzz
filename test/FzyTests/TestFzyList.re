...
describe("Fzy [List]: Match scores should be correct.", ({test, _}) => {
  ...
  test("Doesn't match index when not possible", ({expect, _}) => {
    let result = Array.of_list(fzySearchList(["abc"], "abx", ()));
    expect.equal(result, [||]);
  });

  test("Does match when possible", ({expect, _}) => {
    let result = Array.of_list(fzySearchList(["axbycz"], "abc", ()));
  ...
  test(
    ...
    let result = Array.of_list(fzySearchList(testInputs, testPattern, ()));

    expect.equal(result[0].term, List.nth(testInputs, 0));
  ...
  test("Correctly sorts results for fuzzy match.", ({expect, _}) => {
    ...
    let result =
      Array.of_list(
        fzySearchList(Array.to_list(testInputs), testPattern, ()),
      );
  ...
  test("Better match is picked", ({expect, _}) => {
    let result1 = Array.of_list(fzySearchList(["abcxyz"], "abc", ()));
    let result2 = Array.of_list(fzySearchList(["abcxyz"], "acz", ()));
